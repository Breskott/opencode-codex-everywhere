...
> O plugin também aceita `CODEX_EASY_API_KEY` e `CE_API_KEY`, e usa `OPENAI_API_KEY` como fallback. Confira a base URL em `API Keys` → `Use Key` — se a sua for diferente de `codex-easy.ai`, defina também `CODEX_EVERYWHERE_BASE_URL`.
...
| `OPENAI_API_KEY` | fallback | Usada se nenhuma das acima existir |
...
- **Anthropic** — `claude-fable-5(-1)`, `claude-opus-5`, `claude-sonnet-5`, `claude-opus-4.8/4.7/4.6`, `claude-sonnet-4.6`, `claude-haiku-4.5` (preços = Kiro `0.045x`; `fable-5` usa Max Pool `0.24x` porque o Kiro não tem)
- **Google** — `gemini-3.7-flash`, `gemini-3.6-flash`, `gemini-3.5-flash`, `gemini-3.1-pro(-preview)`, `gemini-3-flash-preview` (preços = Antigravity `0.06x`)
